...
async function updateLogos() {
  console.log('🎨 Updating logos with Simple Icons SVGs...\n');
  ...
  for (const logoUpdate of logoUpdates) {
    try {
      ...
      if (!tool) {
        console.log(`❌ Tool not found: ${logoUpdate.name}`);
        continue;
      }
      ...
      console.log(`✅ Updated ${logoUpdate.name} logo`);
    } catch (error) {
      console.error(`❌ Failed to update ${logoUpdate.name}:`, error);
    }
  }
